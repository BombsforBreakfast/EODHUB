/**
 * Membership / employer country picker.
 * Full ISO 3166-1 alpha-2 list minus adversarial geo deny-list codes.
 */

import { DENIED_GEO_COUNTRY_CODES } from "./geoAccess";
...
/** UN/ISO short English names for selectable countries. */
const ALL_COUNTRIES_UNSORTED: MembershipCountry[] = [
...
export const MEMBERSHIP_COUNTRIES: MembershipCountry[] = ALL_COUNTRIES_UNSORTED
  .filter((c) => !DENIED_GEO_COUNTRY_CODES.has(c.code))
  .sort((a, b) => a.name.localeCompare(b.name));

const MEMBERSHIP_COUNTRY_CODES = new Set(MEMBERSHIP_COUNTRIES.map((c) => c.code));

export const MEMBERSHIP_COUNTRY_HELPER =
  "Select the country where you are based. Some regions are unavailable for EOD-HUB accounts.";
...
/**
 * ATLW Hotline is US-only. Null/unknown country (legacy) still sees it;
 * explicit non-US membership hides it.
 */
