...
// Security Enhancement: Use cryptographically secure random number generation to prevent predictability.
export function makeReferralCode(length = 8): string {
  let code = "";
  const randomValues = new Uint32Array(length);
  const maxValid = 0xffffffff - (0xffffffff % CHARS.length);

  let i = 0;
  while (i < length) {
    crypto.getRandomValues(randomValues);
    for (let j = 0; j < randomValues.length && i < length; j++) {
      if (randomValues[j] <= maxValid) {
