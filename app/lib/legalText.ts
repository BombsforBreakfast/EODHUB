export const LEGAL_EFFECTIVE_DATE = "April 9, 2026";
export const LEGAL_LAST_UPDATED = "June 2, 2026";
export const LEGAL_CONTACT_EMAIL = "Hello@eod-hub.com";

export const TERMS_OF_SERVICE_TEXT = `Terms of Service

Effective Date: ${LEGAL_EFFECTIVE_DATE}
Last Updated: ${LEGAL_LAST_UPDATED}

Welcome to EOD Hub ("EOD Hub," "Platform," "we," "us," or "our"). These Terms of Service ("Terms") govern your access to and use of our website, applications, and services (collectively, the "Services").

By accessing or using the Services, you agree to be bound by these Terms. If you do not agree, do not use the Services.

1. ELIGIBILITY

You must be at least eighteen (18) years of age to access or use the Services.

By using the Services, you represent and warrant that:
- You are at least 18 years old
- You have the legal capacity to enter into a binding agreement
- You will comply with all applicable laws and regulations

EOD Hub community membership is intended for qualified Military Explosive Ordnance Disposal (EOD) personnel and Public Service Bomb Technicians who are in good professional standing, including active, former, retired, reserve, guard, or otherwise credentialed personnel whose background is consistent with the purpose of the Platform.

By creating or maintaining a member account, you represent and warrant that:
- The identity, service history, qualifications, and professional information you provide are accurate and complete
- You meet the membership eligibility standards described above
- You are not misrepresenting your affiliation, credentials, qualifications, employment, service history, or standing within the EOD or Public Service Bomb Technician community

We may request additional information, documentation, community vouches, or other verification at any time. If we determine, in our sole discretion, that you do not meet these eligibility requirements, that your account information is false or misleading, or that your continued participation may compromise the trust, safety, or professional integrity of the Platform, we may immediately remove, suspend, restrict, or terminate your account and access to the Services without prior notice.

If you are using the Services on behalf of an organization, you represent that you have authority to bind that organization.

2. ACCOUNT REGISTRATION AND SECURITY
2.1 Account Creation

To access certain features, you must create an account. You agree to:
- Provide accurate, current, and complete information
- Maintain and update your information as necessary

2.2 Account Responsibility

You are responsible for:
- Maintaining the confidentiality of your login credentials
- All activities that occur under your account

You must notify us immediately of any unauthorized use of your account.

3. DESCRIPTION OF SERVICES

EOD Hub provides a platform for:
- Professional networking and community interaction
- Job postings and employment opportunities
- Business listings and services
- Group-based discussions ("Units")
- Content sharing, including posts, comments, and media

We do not guarantee:
- The accuracy or reliability of user-generated content
- The legitimacy of job postings or business listings
- Employment outcomes or business transactions

4. USER CONTENT
4.1 Ownership

You retain ownership of any content you submit, including:
- Posts
- Comments
- Images
- Profile information

4.2 License Grant

By submitting content, you grant EOD Hub a non-exclusive, worldwide, royalty-free, transferable license to use, display, reproduce, distribute, and modify your content for purposes of operating and improving the Services.

4.3 Content Responsibility

You are solely responsible for your content and any consequences arising from it.

5. ACCEPTABLE USE

You agree not to:
- Violate any applicable laws or regulations
- Post false, misleading, or fraudulent information
- Harass, abuse, or harm other users
- Upload or distribute malware or harmful code
- Attempt to gain unauthorized access to systems or data
- Engage in spam, phishing, or unsolicited advertising
- Impersonate another person or entity

6. PROFESSIONAL AND SENSITIVE CONTENT

Due to the nature of the community, you agree NOT to:
- Share classified, restricted, or sensitive operational information
- Post tactics, techniques, or procedures that could pose real-world risk
- Disclose confidential military, law enforcement, or security-related data

We reserve the right to remove such content immediately.

7. JOBS AND BUSINESS LISTINGS

EOD Hub acts solely as a platform and does not:
- Verify the accuracy of job postings or listings
- Guarantee employment, compensation, or business outcomes

Users are responsible for:
- Conducting their own due diligence
- Evaluating opportunities independently

We are not a party to any agreement between users.

8. PAYMENTS AND SUBSCRIPTIONS
8.1 Paid Features

Certain features may require payment, including subscription plans.

8.2 Billing

By subscribing, you agree:
- To pay all applicable fees
- That payments may be processed through third-party providers (e.g., Stripe)

8.3 Trials

We may offer free trial periods. After the trial:
- Your account may be charged automatically unless canceled

8.4 Refunds

All payments are non-refundable unless required by law.

9. TERMINATION AND SUSPENSION

We may, at our sole discretion:
- Suspend or terminate your account
- Remove content
- Restrict access to Services

Reasons may include:
- Violation of these Terms
- Harmful or abusive behavior
- Legal or security concerns
- Failure to meet or maintain Platform eligibility requirements
- False, misleading, or unverifiable identity, affiliation, credential, or professional-standing information

You may terminate your account at any time.

10. INTELLECTUAL PROPERTY

All Platform content, excluding user content, including logos, design, code, and features are the property of EOD Hub and protected by applicable laws.

You may not copy, modify, or distribute our intellectual property without permission.

11. THIRD-PARTY SERVICES

The Services may include links or integrations with third-party services.

We are not responsible for:
- Third-party content
- Their policies or practices
- Any damages resulting from their use

Use of third-party services is at your own risk.

12. DISCLAIMERS

THE SERVICES ARE PROVIDED "AS IS" AND "AS AVAILABLE."

WE DISCLAIM ALL WARRANTIES, INCLUDING:
- MERCHANTABILITY
- FITNESS FOR A PARTICULAR PURPOSE
- NON-INFRINGEMENT

We do not guarantee:
- Continuous availability
- Error-free operation
- Accuracy of information

13. LIMITATION OF LIABILITY

TO THE MAXIMUM EXTENT PERMITTED BY LAW, EOD HUB SHALL NOT BE LIABLE FOR:
- INDIRECT, INCIDENTAL, OR CONSEQUENTIAL DAMAGES
- LOSS OF PROFITS, DATA, OR OPPORTUNITIES
- USER INTERACTIONS OR DISPUTES

OUR TOTAL LIABILITY SHALL NOT EXCEED THE AMOUNT PAID BY YOU (IF ANY) IN THE LAST 12 MONTHS.

14. INDEMNIFICATION

You agree to indemnify and hold harmless EOD Hub from any claims, damages, or liabilities arising from:
- Your use of the Services
- Your content
- Your violation of these Terms

15. GOVERNING LAW

These Terms are governed by the laws of the State of North Carolina, without regard to conflict of law principles.

16. DISPUTE RESOLUTION
16.1 Informal Resolution

You agree to first attempt to resolve disputes informally by contacting us.

16.2 Arbitration

If unresolved, disputes may be resolved through binding arbitration, except where prohibited by law.

17. MODIFICATIONS TO TERMS

We reserve the right to modify these Terms at any time.

We will provide notice by:
- Updating the "Last Updated" date
- Posting updates within the Platform

Continued use of the Services constitutes acceptance.

18. ENTIRE AGREEMENT

These Terms constitute the entire agreement between you and EOD Hub regarding the Services.

19. CONTACT INFORMATION

For questions or concerns:
${LEGAL_CONTACT_EMAIL}`;

export const PRIVACY_POLICY_TEXT = `Privacy Policy

Effective Date: ${LEGAL_EFFECTIVE_DATE}
Last Updated: ${LEGAL_LAST_UPDATED}

Your privacy matters. This policy explains how we collect and use your data.

1. INFORMATION WE COLLECT

Account Information
- Name
- Email address
- Profile details (experience, unit, etc.)

User Content
- Posts, comments, images
- Messages (if messaging features are used)

Technical Data
- IP address
- Device/browser info
- Usage behavior

2. HOW WE USE YOUR INFORMATION

We use your data to:
- Provide and improve the Platform
- Enable communication between users
- Match users with jobs and opportunities
- Send important updates and notifications
- Ensure security and prevent abuse

3. DATA SHARING

We do NOT sell your data.

We may share data with:
- Service providers (e.g., hosting, authentication like Supabase)
- Payment processors (for subscriptions)
- Law enforcement if required

4. USER PROFILES AND VISIBILITY

Some profile information may be visible to other users.
Content you post is visible within the Platform.
You control what you share.

5. DATA STORAGE AND SECURITY

We use industry-standard security measures, including:
- Encrypted connections (HTTPS)
- Secure authentication systems

However, no system is 100% secure.

6. YOUR RIGHTS

You may:
- Update or delete your profile
- Request deletion of your data
- Opt out of non-essential communications

7. COOKIES AND TRACKING

We may use cookies to:
- Improve user experience
- Track usage patterns

8. CHILDREN'S PRIVACY

This Platform is not intended for users under 18.

9. CHANGES TO THIS POLICY

We may update this policy. Continued use means acceptance.

10. CONTACT

For privacy questions:
${LEGAL_CONTACT_EMAIL}`;

export const COMMUNITY_GUIDELINES_TEXT = `Community Guidelines

Effective Date: ${LEGAL_EFFECTIVE_DATE}
Last Updated: ${LEGAL_LAST_UPDATED}

EOD Hub is built for professional connection, trust, and shared experience. These guidelines ensure the platform stays valuable.

Membership is reserved for qualified Military EOD personnel and Public Service Bomb Technicians who are in good professional standing, along with approved employer and administrative accounts that support that community. Do not create, maintain, or use an account if you do not meet the applicable eligibility requirements or if you are misrepresenting your identity, affiliation, credentials, qualifications, service history, employment, or professional standing.

1. BE PROFESSIONAL
- Treat others with respect
- Disagree without being disrespectful
- Keep conversations constructive

2. KEEP IT RELEVANT
- Stay on topic within posts and groups
- Avoid spam or irrelevant promotions
- Use appropriate groups ("Units") for discussions

3. NO HARASSMENT OR HATE

Zero tolerance for:
- Harassment
- Threats
- Hate speech
- Bullying

4. NO MISINFORMATION

Do not post false or misleading information, especially for technical and safety-related discussions.

5. RESPECT OPERATIONAL SENSITIVITY

Do NOT share sensitive or classified information.
Avoid posting tactics, techniques, or procedures that could create risk.

6. BUSINESS AND JOB POSTING INTEGRITY
- Only post legitimate opportunities
- Clearly represent compensation, requirements, and expectations
- No scams or deceptive listings

7. CONTENT STANDARDS

Allowed:
- Professional discussions
- Industry humor (within reason)
- Photos relevant to work/community

Not allowed:
- Explicit content
- Illegal activity
- Graphic or unsafe material

8. GROUPS ("UNITS") BEHAVIOR
- Keep discussions aligned with the group's purpose
- Moderators may remove off-topic content

9. ENFORCEMENT

Violations may result in:
- Content removal
- Account warnings
- Suspension or permanent ban

Accounts found to be ineligible, falsely represented, misleading, or otherwise inconsistent with the membership standards of EOD Hub may be removed immediately to protect the trust, safety, and integrity of the Platform and its community.

10. BUILD THE CULTURE

This is YOUR community.
Help new users, share knowledge, and strengthen the network.

Questions about moderation or enforcement:
${LEGAL_CONTACT_EMAIL}`;
