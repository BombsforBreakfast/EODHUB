...
//
// What it does:
//   - Waits for auth to finish loading, then opens an analytics session + first
//     page_view (so authenticated users are not stuck as anonymous).
...
function isUserExcludedFromAnalytics(user: User | null): boolean {
  return isExcludedAnalyticsUser(user);
}
...
export default function AnalyticsTracker() {
  ...
  // ── Start session after auth resolves, on first allowed pathname ───────────
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    let cancelled = false;
    void (async () => {
      if (isUserExcludedFromAnalytics(currentUserRef.current)) return;
      ...
      try {
        ...
        if (json.session_id && json.page_view_id) {
          ...
          identifiedRef.current = Boolean(accessTokenRef.current);
  ...
  // ── Attach user_id if session started anonymous then auth arrived ──────────
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (authLoading) return;
    ...
    if (isUserExcludedFromAnalytics(user)) return;
  ...
  useEffect(() => {
    ...
    const flush = (action: "heartbeat" | "end") => {
      ...
      try {
        ...
        // session row already has user_id from "start" / "identify".
