-- Safe to re-run: creates anything missing from the initial BSM schema.
-- Use this if 20260618120000_bsm_initial_schema.sql failed partway (e.g. profiles already exists).
...
drop trigger if exists on_auth_user_created_bsm on auth.users;
create trigger on_auth_user_created_bsm
  after insert on auth.users
  for each row execute function public.handle_bsm_new_user();

create table if not exists public.bsm_high_scores (
...
create index if not exists bsm_high_scores_user_id_idx on public.bsm_high_scores (user_id);
create index if not exists bsm_high_scores_leaderboard_idx
...
drop policy if exists "bsm_high_scores_select_own" on public.bsm_high_scores;
create policy "bsm_high_scores_select_own"
  on public.bsm_high_scores for select to authenticated using (user_id = auth.uid());

drop policy if exists "bsm_high_scores_insert_own" on public.bsm_high_scores;
create policy "bsm_high_scores_insert_own"
  on public.bsm_high_scores for insert to authenticated with check (user_id = auth.uid());

drop policy if exists "bsm_high_scores_update_own" on public.bsm_high_scores;
create policy "bsm_high_scores_update_own"
  on public.bsm_high_scores for update to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

create table if not exists public.bsm_completions (
  user_id uuid not null references auth.users (id) on delete cascade,
  level_id text not null,
...
drop policy if exists "bsm_completions_select_own" on public.bsm_completions;
create policy "bsm_completions_select_own"
  on public.bsm_completions for select to authenticated using (user_id = auth.uid());

drop policy if exists "bsm_completions_insert_own" on public.bsm_completions;
create policy "bsm_completions_insert_own"
  on public.bsm_completions for insert to authenticated with check (user_id = auth.uid());

drop policy if exists "bsm_completions_update_own" on public.bsm_completions;
