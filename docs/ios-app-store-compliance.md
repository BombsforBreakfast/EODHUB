...
**Camera / photo library:** `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription`, `NSMicrophoneUsageDescription`, and `NSPhotoLibraryAddUsageDescription` are required in the native shell. Feed photo capture on iOS uses `@capacitor/camera` (not the WKWebView file input camera path, which crashes). Changes require a **new TestFlight build** — they are not applied by a Vercel deploy alone.
